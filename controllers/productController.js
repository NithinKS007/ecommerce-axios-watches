...
const addProduct = async (req,res) =>{
    ...
    try {
        ...
        if (!name || !brand || !category || !dialShape || !displayType || !salesPrice || !strapMaterial || !strapColor || !stock || !description || !targetGroup) {
        ...
        if (!brandFromCollection) {
            return res.status(400).json({ message: "Brand not found", success: false });
        }
...
const loadEditProduct = async (req,res) =>{
    ...
    try {
        ...
        const [categoriesData, brandsData, productDataToEdit] = await Promise.all([
           ...
           categories.find({}),
           brands.find({}),
           products.findOne({_id:productId}).populate('category').populate('brand')
