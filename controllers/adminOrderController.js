...
const loadOrderDetailsPage = async (req, res) => {
  try {
    ...
    const userOrderDataPromise = orders
      ...
      .populate("user")
      .exec();
    const transactionDetailsPromise = transaction
  ...
  try {
    ...
    if (
      ...
      const productsAmountThatHaventRefunded = walletData.transactions
        ...
        .filter(
          (walletRecord) =>
            walletRecord.type === "credit" &&
            walletRecord.orderId.equals(order._id)
        )
        .reduce(
          (remainingRefundAmount, walletRecord) =>
            remainingRefundAmount + walletRecord.amount,
          0
        );
...
const loadReturnedOrder = async (req, res) => {
  try {
    ...
    const skip = (pageNumber - 1) * perPageData;
    const findReturnedOrdersPromise = returnUserOrder
      .find(query)
      .populate("userId")
      .skip(skip)
      .limit(perPageData)
    ...
    const [totalReturnedOrders, returnedOrderData] = await Promise.all([
      countReturnedOrdersPromise,
      findReturnedOrdersPromise,
    ]);

    const totalPages = Math.max(
      1,
      Math.ceil(totalReturnedOrders / perPageData)
    );
    pageNumber = Math.max(1, Math.min(pageNumber, totalPages));
...
const approveReturn = async (req, res) => {
  try {
    ...
    const productToUpdate = orderData.items.find(
    ...
    const allProductsInitiated = orderData.items.every(
      (product) => product.orderProductStatus === "returnApproved"
    );
...
const rejectReturn = async (req, res) => {
  try {
    ...
    const allProductsInitiated = orderData.items.every(
      (product) => product.orderProductStatus === "returnRejected"
    );
