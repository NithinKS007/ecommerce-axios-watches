...
const loadCart = async (req,res) =>{
    ...
    try {
             ...
             const [cartDetails, couponDetails, isAlreadyUsedCoupons] = await Promise.all([
...
const addToCart = async (req,res) =>{
    ...
    try {
      ...
      const [findExistingCart, productDetails] = await Promise.all([
            ...
            products.findOne({ _id: productId }, { salesPrice: 1, productSalesPriceAfterOfferDiscount: 1, _id: 0 }).exec()
        ]);
        ...
        if(!findExistingCart){
            ...
            const cartItem = new cart({
                ...
                items:[{
                    ...
                    product:productId,
                    price:salesPrice,
                    productSalesPriceAfterOfferDiscount:productSalesPriceAfterOfferDiscount
            ...
            await cart.updateOne({user:currentUser?._id},{$push:{items:{product:productId,price:salesPrice, productSalesPriceAfterOfferDiscount:productSalesPriceAfterOfferDiscount }}})
...
const removeFromCart = async (req, res) => {
    ...
    try {
     ...
     const [deletedProductFromCart, productName ] = await Promise.all([
        cart.updateOne({ user: currentUser._id }, { $pull: { items: { product: productId } } }),
