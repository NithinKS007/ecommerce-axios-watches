...
const loadHome = async (req,res) => {
    ...
    try {
        const [
            ...
            categories.find({}),
          products.find({}).populate('brand').populate('category').sort({createdAt:-1}) .skip(skip)
        .limit(perPageData)
        .exec()
...
const getNewArrivals = async () =>{
    ...
    try {
        ...
        return products.find({}).sort({createdAt:-1}).limit(4).populate('brand').populate('category').exec()
...
const loadShowCase = async (req,res) =>{
    ...
    try {
        ...
        const [categoriesArray, brandArray, totalProducts, productsArray, latestProducts] = await Promise.all([
            categories.find({ isBlocked: false }).exec(),
            brands.find({ isBlocked: false }).exec(),
            products.countDocuments({ targetGroup: targetGroup }).exec(),
            products.find({ targetGroup: targetGroup })
                .populate('brand')
                .populate('category')
                .sort({ createdAt: -1 })
...
const advancedSearch = async (req, res) => {
    ...
    const categoriesArray = categories ? categories.split(',').map(id => ObjectId.createFromHexString(id)) : [];
    const brandsArray = brands ? brands.split(',').map(id => ObjectId.createFromHexString(id)) : [];
...
const loadProductDetails = async (req,res) =>{
    ...
    try {
        ...
        const [existingCartItem, productDetails, productInWishList] =  await Promise.all([

        cart.findOne({user:currentUser?._id,items:{$elemMatch:{product:productId}}}),

        products.findById({_id:productId}).populate('category').populate('brand'),
