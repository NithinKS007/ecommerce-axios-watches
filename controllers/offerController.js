...
const loadCategoryOffer = async (req, res) => {
    ...
    try {
        const [totalOfferAppliedCategories, offerAppliedCategories] = await Promise.all([

          categories.countDocuments({ "categoryOffer.offerName": { $exists: true } }),
        
          categories.find({ "categoryOffer.offerName": { $exists: true } })
         .sort({ createdAt: -1 }) 
         .skip(skip)
         .limit(perPageData)
         .exec()
    ...
    try {
       ...
       const updatedCategoryOfferStatus = await categories.findByIdAndUpdate(
        ...
        for(const product of productsInCategory){
            ...
            let categoryOfferdiscountAmount = (categoryData.categoryOffer.offerDiscountPercentage / 100) * s
...
const loadProductOffer = async (req, res) => {
    ...
    try {
        ...
        const [totalOfferAppliedProducts, offerAppliedProducts] = await Promise.all([ 
         products.countDocuments({ 'productOffer.offerDiscountAmount': { $exists: true } }),
         products.find({ 'productOffer.offerDiscountAmount': { $exists: true } })
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(perPageData)
                .exec()
